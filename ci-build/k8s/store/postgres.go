...
func PGInit(bus *dataBus.DataBus) {
	...
	if bus.Postgres.Passwd == "" {
		connStr = fmt.Sprintf("postgres://%s@%s:%d/%s?sslmode=verify-full", bus.Postgres.User, bus.Postgres.Addr, bus.Postgres.Port, bus.Postgres.Name)
	} else {
		connStr = fmt.Sprintf("postgres://%s:%s@%s:%d/%s?sslmode=verify-full", bus.Postgres.User, bus.Postgres.Passwd, bus.Postgres.Addr, bus.Postgres.Port, bus.Postgres.Name)
	}
